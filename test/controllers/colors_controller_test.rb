...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference("Color.count") do
      post colors_url, params: { color: { collection: @color.collection, name: @color.name } }, as: :json
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { collection: @color.collection, name: @color.name } }, as: :json
    assert_response :success
  end
